...
function get20s(array){
  ...
  for(let index=0;index<array.length; index++){
    let birth=array[index].years.slice(0,4);
    let death=array[index].years.slice(7,11);
    if (birth >= 1900 && death <= 2000){
      dead.push(array[index].name)
    }
...
function addArtist(array){
     const mySelf= {
       ...
       "bio": "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Similique numquam dolorem amet voluptate animi suscipit illo tempora possimus recusandae quaerat!"}  
     array.push(mySelf);
      return array;
    }
...
function lotsOfArt(array){
  ...
  for(let index=0;index<array.length; index++){
    if (artists[index].paintings > 100){
       overHundred.push(array[index].name); 
          }
      }
      return overHundred;
    }
