...
function get20s(array){
 ...
 for(let i = 0; i < array.length; i++){
  ...
  const life = years.slice(0,4); 
  const death = years.slice(7,11);
  if( life > "1899" && death < "2000"){
lifetime.push(array[i].name);
  }
 }
 return lifetime;
}
...
function addArtist(array){
    array.push({id:20, name:"David Martin", years:"1981 - 2021", genre:"Web Design",  nationality:"North America", bio:"man im really glad I flexed becuase this all makes a lot more sense to me now " });
    return array;
  }
...
function lotsOfArt(array){
  ...
  for (let i = 0; i < array.length; i++){
    if(array[i].paintings >= 100){
      paintingCount.push(array[i].name)
    }
  }
